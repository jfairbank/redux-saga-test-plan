...
test('handles throttle effect', () => {
  testSaga(mainSaga)
    .next()
    .throttle(500, 'FOO', backgroundSaga, 42);
});
...
test('throws if wrong delay', () => {
  expect(_ => {
    testSaga(mainSaga)
      .next()
      .throttle(250, 'FOO', backgroundSaga, 42);
  }).toThrow();
});
...
test('throws if wrong pattern', () => {
  expect(_ => {
    testSaga(mainSaga)
      .next()
      .throttle(500, 'BAR', backgroundSaga, 42);
  }).toThrow();
});
...
test('throws if wrong saga', () => {
  expect(_ => {
    testSaga(mainSaga)
      .next()
      .throttle(500, 'FOO', otherBackgroundSaga, 42);
  }).toThrow();
});
...
test('throws if args are wrong', () => {
  expect(_ => {
    testSaga(mainSaga)
      .next()
      .throttle(500, 'FOO', backgroundSaga, 43);
  }).toThrow();
});
